#' @export
#'
susie_suff_stat_adjustXtX = function (XtX, Xty, yty, n,
  ...
  s = init_setup(0,p,L,scaled_prior_variance,residual_variance,prior_weights,
                 null_weight,yty/(n-1),standardize,correct_zR_discrepancy = TRUE)
  s$Xr = NULL
  s$XtXr = rep(0,p)
  s$XtX = XtX
  ...
  for (i in 1:max_iter) {
    ...
    s = update_each_effect_ss_adjustXtX(Xty,s,estimate_prior_variance,
                              estimate_prior_method,check_null_threshold, check_outlier = i > 1, dist_thres = dist_thres, LD_thres = LD_thres, 
                              likelihood_thres = likelihood_thres, weight_thres = weight_thres, region = region, outlier_method = outlier_method, use_post_mean_l = use_post_mean_l)
    if(check_prior){
      if(any(s$V > 100*(zm^2))){
	...
	if (!s$force_iterate) {
    	...
    	elbo[i+1] = get_objective_ss(s$XtX,Xty,s,yty,n)
  ...
  elbo = elbo[!is.infinite(elbo) & !is.na(elbo)] # Remove first (infinite) entry, and trailing NAs.
  s$elbo = elbo
  s$niter = i
  ...
  if (!is.null(coverage) && !is.null(min_abs_corr)) {
    if(any(!(diag(s$XtX) %in% c(0,1)))){
      s$sets = susie_get_cs(s,coverage = coverage,Xcorr = muffled_cov2cor(s$XtX),
                            min_abs_corr = min_abs_corr,
                            check_symmetric = FALSE,
                            n_purity = n_purity)
    }else
      s$sets = susie_get_cs(s,coverage = coverage,Xcorr = s$XtX,
                            min_abs_corr = min_abs_corr,
                            check_symmetric = FALSE,
                            n_purity = n_purity)
  ...
  if (!is.null(colnames(s$XtX))) {
    variable_names = colnames(s$XtX)
    if (!is.null(null_weight)) {
  ...
  ## refine is not working for adjustXtX
  if (refine) {
    if (!missing(s_init) && !is.null(s_init))
